...
void reduce(const real *d_A, size_t size, real *h_result)
{
    ...
    kernel<<<grid_size, block_size, block_size / 32 * real_size>>>(d_A, size, d_B);
    CHECK(cudaGetLastError());
    CHECK(cudaDeviceSynchronize());

    unsigned block_size2 = 512;
    kernel<<<1, block_size2, block_size2 / 32 * real_size>>>(d_B, grid_size, d_B);
    CHECK(cudaGetLastError());
    CHECK(cudaDeviceSynchronize());
